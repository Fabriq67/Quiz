...
class CulturaService {
  ...
  static Future<List<Pregunta>> _loadAll() async {
    ...
    final String jsonString =
        await rootBundle.loadString("assets/questions/cultura.json");

    final List<dynamic> data = json.decode(jsonString);
    _cachedPreguntas = data.map((e) => Pregunta.fromJson(e)).toList();

    final faciles =
        _cachedPreguntas!.where((p) => p.dificultad == "facil").length;
    final medias =
        _cachedPreguntas!.where((p) => p.dificultad == "media").length;
    final dificiles =
        _cachedPreguntas!.where((p) => p.dificultad == "dificil").length;
    final muyDificiles =
        _cachedPreguntas!.where((p) => p.dificultad == "muy_dificil").length;
  ...
  static Future<List<Pregunta>> obtenerPreguntasBloque(int blockId) async {
    ...
    final muyDificiles =
        todas.where((p) => p.dificultad == "muy_dificil").toList();
    ...
    if (blockId == 1) {
      ...
      if (seleccionadas.length < necesarias) {
        print(
            "❌ BLOQUE 1: Solo hay ${seleccionadas.length}/$necesarias preguntas fáciles");
      }
    } else if (blockId == 2) {
      necesarias = 5;
      ...
      if (seleccionadas.length < necesarias) {
        print(
            "❌ BLOQUE 2: Solo hay ${seleccionadas.length}/$necesarias preguntas medias");
      }
    } else if (blockId == 3) {
      necesarias = 10;
      ...
      if (seleccionadas.length < necesarias) {
        print(
            "❌ BLOQUE 3: Solo hay ${seleccionadas.length}/$necesarias preguntas difíciles");
      ...
      if (seleccionadas.length < necesarias) {
        print(
            "❌ BLOQUE 4 (JEFE): Solo hay ${seleccionadas.length}/$necesarias preguntas muy difíciles");
